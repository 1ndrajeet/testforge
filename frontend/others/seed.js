...
const CONFIG = {
  ...
  EXAM_CENTER_ID: 'a3e5c386-efbb-373e-36d0-1bc812bde9d2',
  ORG_NAME: 'MMCOE',
  ORG_SLUG: 'mmcoe',
  EC_CODE: '1740',
...
function generateAccountId() {
  return generateUuidFromString(`${CONFIG.USER_EMAIL}_credential`);
}
...
function calculateExamDays() {
  ...
  console.log(`📅 Generated ${uniqueDates.length} exam days`);
}
...
async function generatePostgresSQL() {
  ...
  if (CONFIG.INSERT_USER) {
    ...
    writeLine('-- ============================================');
    writeLine('-- 0. User & Account (for login)');
    writeLine('-- ============================================');
    writeLine('');
...
async function main() {
  ...
  console.log(`   1. psql -d your_database -f ${outputFile}`);
  console.log(`   2. Login with: ${CONFIG.USER_EMAIL} / ${CONFIG.USER_PASSWORD}`);
}
