...
export async function getAdminStats() {
  try {
    ...
    const [
      ...
      db.select({ count: count() }).from(organizations).where(eq(organizations.subscriptionTier, 'trial')),
...
export async function getOrganizations(params?: {
  ...
  try {
    ...
    const [orgs, total] = await Promise.all([
      ...
      db.select({ count: count() }).from(organizations).where(conditions.length ? and(...conditions) : undefined),
    ]);
...
export async function getUsers(params?: {
  ...
  try {
    ...
    const [userList, total] = await Promise.all([
      ...
      db.select({ count: count() }).from(users).where(conditions.length ? and(...conditions) : undefined),
    ]);
...
export async function getEmailLogs(params?: {
  ...
  try {
    ...
    const [logs, total] = await Promise.all([
      ...
      db.select({ count: count() }).from(emailLogs).where(conditions.length ? and(...conditions) : undefined),
    ]);

    // Fetch related data manually
    const logsWithDetails = await Promise.all(
      logs.map(async (log) => {
        const user = log.userId ? await db.query.users.findFirst({
          where: eq(users.id, log.userId),
          columns: { id: true, name: true, email: true },
        }) : null;
        const examCenter = log.examCenterId ? await db.query.examCenters.findFirst({
...
export async function getPayments(params?: {
  ...
  try {
    ...
    const [paymentList, total] = await Promise.all([
      ...
      db.select({ count: count() }).from(payments).where(conditions.length ? and(...conditions) : undefined),
    ]);

    // Fetch organization for each payment
    const paymentsWithOrg = await Promise.all(
      paymentList.map(async (payment) => {
        const org = payment.orgId ? await db.query.organizations.findFirst({
...
export async function getAuditLogs(params?: {
  ...
  try {
    ...
    const [logs, total] = await Promise.all([
      ...
      db.select({ count: count() }).from(auditLogs).where(conditions.length ? and(...conditions) : undefined),
    ]);
...
export async function getBulkEmailRecipients(params?: {
  orgIds?: string[];
  roles?: string[];
}) {
  try {
...
export async function getSystemHealth() {
  try {
    ...
    const emailFailures = checks[1].status === 'fulfilled' ? Number(checks[1].value[0]?.count || 0) : 0;
    const pendingUploads = checks[2].status === 'fulfilled' ? Number(checks[2].value[0]?.count || 0) : 0;
