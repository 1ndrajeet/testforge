...
async function checkTableExists(db: NodePgDatabase<Record<string, never>>, tableName: string): Promise<boolean> {
...
async function getTableCount(db: NodePgDatabase<Record<string, never>>, tableName: string): Promise<number> {
  const result = await db.execute(sql`
...
async function importSqlFile(db: NodePgDatabase<Record<string, never>>, filePath: string): Promise<number> {
...
async function seedFile(db: NodePgDatabase<Record<string, never>>, filename: string): Promise<SeedResult> {
